...
from procthor_v2.constants import (
    MARGIN,
...
from procthor_v2.databases import ProcTHORDatabase, get_spawnable_asset_group_info
from procthor_v2.utils.types import Object, Split, Vector3
...
def default_add_floor_objects(
    ...
    room_object_copy = copy.deepcopy(room_object)
    for room in partial_house.rooms.values():
        ...
        room_object_list = list(room_object_copy.keys())
        priority_asset_types = list(set(copy.deepcopy(pt_db.PRIORITY_ASSET_TYPES[room.room_type]) + room_object_list))
        ...
        for i in range(max_floor_objects):
            ...
            for asset_type in added_asset_types:
                if asset_type in room_object_copy:
                    del room_object_copy[asset_type]
